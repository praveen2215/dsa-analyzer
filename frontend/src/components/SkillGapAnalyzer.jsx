...
// Build a flat lookup from ALL your actual topic data — same approach as DifficultyPredictor
...
// Get real mastery % for a topic slug — tries multiple key variations
function getTopicMastery(slug, lookup) {
  ...
  if (solved === null) return 0  // topic not found — assume 0 mastery
...
function computeReadiness(company, data) {
  ...
  // Topic score — weighted average mastery using REAL data
  const topicScores = cfg.topics.map(t => ({
    ...t,
    mastery: getTopicMastery(t.slug, lookup),
  }))
  ...
  // Volume score — how many problems solved vs minimum required
  const volScore  = Math.min(100, Math.round((solved.total / cfg.minSolved) * 100))

  // Hard score — hard problems vs minimum required
  const hardScore = Math.min(100, Math.round((solved.hard  / cfg.minHard)   * 100))

  // Overall readiness — topic mastery drives most of it
  const overall = Math.round(topicScore * 0.50 + volScore * 0.30 + hardScore * 0.20)

  // Priority gaps — topics below 70% mastery, sorted by impact (weight × gap)
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
        ...
        <div>
          ...
          <div style={{ padding:"12px 14px", background:"var(--surface2)", borderRadius:8, border:"0.5px solid var(--border)" }}>
            ...
            <div style={{ marginTop:8, paddingTop:8, borderTop:"0.5px solid var(--border)", fontSize:11, color:"var(--text3)" }}>
              Readiness = topic mastery (50%) + volume (30%) + hard problems (20%)
            </div>
          </div>
        </div>
